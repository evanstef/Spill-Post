<x-app-layout>
    ...
    <x-layout-content class="{{ request()->routeIs('post.hashtag.show') ? 'w-full sm:w-[80%] mr-4' : 'w-full sm:w-[60%]' }} rounded-lg mx-4 sm:mx-0">
            ...
            @else
                {{-- konten utama --}}
                {{-- pengecekan bila berada di halaman /post/hashtag/{hashtag} --}}
                ...
                {{-- menampilkan nama hashtag --}}
                @if (request()->routeIs('post.hashtag.show'))
                    <h1 class="text-xs sm:text-sm md:text-base lg:text-lg px-3 sm:px-4 xl:px-6 mt-4">All Post With Hashtag <span class="font-bold">{{ $hashtag }}</span></h1>
                @endif
